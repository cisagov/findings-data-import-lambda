...
def test_basic_validation_v1():
    ...
    with open("tests/artifacts/field_map.json") as fm_file:
    ...
    with open("tests/artifacts/expected_dhs_v1.json") as json_file:
...
def test_basic_validation_v2():
    ...
    with open("tests/artifacts/expected_dhs_v2.json") as json_file:
        expected_findings = json.load(json_file)
        result = fdi.extract_findings(
            findings_data=expected_findings, field_map_dict=basic_field_map
        )
...
def test_database_update():
    """Test database update is called correctly against our mocked db."""
